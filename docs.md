...
3. При событии `raided` код готовит данные рейда и пробует обогатить их через Helix API:
...
4. Рейд кладется в `raidQueue`, даже если Twitch API недоступен, чтобы alert не пропадал полностью.
...
6. Если данные стрима и канала не найдены, используются fallback-тексты из `config.js`.
